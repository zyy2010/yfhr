...
(function (factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        // Node / CommonJS
        factory(require('static/lib/jquery'), require('ChineseDistricts'));
    } else {
        // Browser globals.
    ...
    CityPicker.prototype = {
        ...
        bind: function () {
            ...
            this.$element.on('change', (this._changeElement = $.proxy(function () {
                ...
                if(this.$element.val().trim()==''){//空值时才可显示
                    this.needBlur = true;
                    this.open();
                }
            }, this))).on('blur', (this._blurElement = $.proxy(function () {
                var el=this.$element;
                if(el.hasClass('easyui-validatebox')){
                    setTimeout(function(){el.validatebox('validate')},1000)
                }
                if (this.needBlur) {
                    this.needBlur = false;
                    this.close(true);
                }
            }, this))).on('keyup',(this._keydownElement =$.proxy(function(){
                    this.close(true);
            },this)));
        ...
        getVal: function () {
            ...
            this.$dropdown.find('.city-select')
                ...
                // setTimeout(function(){
                //     var startcity=$('.startcity').val(),endcity=$('.endcity').val();
                //     //计算始发跟到站距离
                //    if( startcity && endcity){
                //         startcity=startcity.split('/');
                //         endcity=endcity.split('/');
                //         // console.log(startcity[startcity.length-1],endcity[endcity.length-1])
                //         transit.search(startcity[startcity.length-1] , endcity[endcity.length-1]);
                //     } 
                // },0)
            return text;
        },
